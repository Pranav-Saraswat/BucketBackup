...
class AnomalyDetector {
    ...
    async analyzeJob(jobId) {
        ...
        // 2. File Size Anomaly
        const sizes = logs.map(l => Number(l.fileSize || 0)).filter(s => s > 0);
        if (sizes.length >= 2) {
            const lastSize = sizes[0];
            const avgSize = sizes.slice(1).reduce((a, b) => a + b, 0) / (sizes.length - 1);
        ...
        if (durations.length >= 2) {
            ...
            const avgDuration = durations.slice(1).reduce((a, b) => a + b, 0) / (durations.length - 1);
